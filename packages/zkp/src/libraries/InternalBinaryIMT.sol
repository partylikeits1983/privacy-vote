...
library InternalBinaryIMT {
    ...
    function _createProof(
        BinaryIMTData storage self,
        uint256 leafIndex // Now we need the index of the leaf for which to create the proof.
    ) internal view returns (uint256[] memory proofSiblings, uint8[] memory proofPathIndices) {
        uint256 depth = self.depth;

        if (leafIndex >= self.numberOfLeaves) {
            revert LeafIndexOutOfRange();
        }

        // Initialize arrays to store the proof.
        proofSiblings = new uint256[](depth);
        proofPathIndices = new uint8[](depth);

        uint256 index = leafIndex;
        for (uint8 i = 0; i < depth;) {
            uint8 pathIndex = uint8(index & 1);
            proofPathIndices[i] = pathIndex;

            // Determine the sibling's position and hash.
            if (pathIndex == 0) {
                // If our node is the left child, the sibling is on the right.
                if (index + 1 < self.numberOfLeaves) {
                    // If the sibling index is within bounds, use the stored right hash.
                    proofSiblings[i] = self.lastSubtrees[i][1];
                } else {
                    // Otherwise, use the zero hash for this depth.
                    proofSiblings[i] = self.useDefaultZeroes ? _defaultZero(i) : self.zeroes[i];
                }
            } else {
                // If our node is the right child, the sibling is on the left.
                // The left sibling hash is always stored since it's filled first.
                proofSiblings[i] = self.lastSubtrees[i][0];
            }

            index >>= 1; // Move up the tree to the next level.
        ...
        return (proofSiblings, proofPathIndices);
    }
}
